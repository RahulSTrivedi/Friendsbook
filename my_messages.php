  ...
  <body>
	  ...
	  <br>
		...
		<div id="timeline">
				...
				<?php
				$user_id=$_GET['u_id'];
				$query  = "SELECT * FROM messages WHERE sender_id='$user_id' OR reciever_id='$user_id' ORDER BY 1 DESC";
				$result = $conn->query($query); 
				$rows = $result->num_rows;
				// See photo for diagram of array x
				$x = array();			
				for ($j = 0 ; $j < $rows ; ++$j)
				{
						$result->data_seek($j);
						$row = $result->fetch_array(MYSQLI_NUM);
						/*
							We determine who is the sender and who is the reciever
							in the code below
						*/
						if($user_id==$row[1])
							$otherperson = $row[2];
						if($user_id==$row[2])
							$otherperson = $row[1];
					/* if $otherperson is not in array x then this statement will
					insert it in array x*/
						if(!in_array($otherperson, $x))
							$x[$row[0]] = $otherperson;
				}
				/* By the time above for loop ends array x has all the otherid with whom
				 userid has done conversation and also their latest message id */

				$query1  = "SELECT * FROM messages WHERE sender_id='$user_id' OR reciever_id='$user_id' ORDER BY 1 DESC";
